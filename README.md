...
- If you need to use SSH for making changes to the repo then you must have two things.\
- One is private key and public key.\
- To generate one use

```ssh
ssh-keygen -t ed25519 -C "youremail@example.com" -f "path"
```
> You can mention `alias` name to the file in the path itself.

- To see the `public key` use
...
> Default location of `.ssh` file is C:\user\name\\.ssh

- Then add the public keys to the respective repositories.
- In the local machine now we need to sset the ssh key for the current local repository alone.

```git
git config core.sshCommand "ssh -i ~/.ssh/id_ed25519_name IdentitiesOnly=yes"
```
